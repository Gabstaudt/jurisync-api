...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { rows } = await q(
    `SELECT l.*, u.name AS user_name
     FROM contract_lawyers cl
     JOIN lawyers l ON l.id = cl.lawyer_id
     LEFT JOIN users u ON u.id = l.user_id
     WHERE cl.contract_id = $1 AND cl.ecosystem_id = $2
     ORDER BY l.name ASC`,
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await q(
    `INSERT INTO contract_lawyers (contract_id, lawyer_id, ecosystem_id)
     VALUES ($1, $2, $3)
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await q(
    `DELETE FROM contract_lawyers
     WHERE contract_id = $1 AND lawyer_id = $2 AND ecosystem_id = $3`,
    [id, lawyerId, session.user.ecosystemId],
  );
